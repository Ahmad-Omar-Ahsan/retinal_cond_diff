...
class Pretrained_LightningDDPM_monai(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.classification:
            ...
            classification_logits_mean = torch.mean(classification_logits, dim=-1, keepdim=True)
            classification_logits_std = torch.std(classification_logits, dim=-1, keepdim=True)
    ...
    def test_step(self,batch,batch_idx):
        ...
        # for test_image, test_label,filename in zip(image_list, label_list,filenames):
            ...
            for r in range(self.runs):
                timesteps = torch.randint(0, self.scheduler_DDPM.num_train_timesteps,(1,),device=self.device).long()
