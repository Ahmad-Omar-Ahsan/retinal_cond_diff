...
class EfficientNet_B3(pl.LightningModule):
    def __init__(self, config):
        ...
        self.model.classifier[1] = nn.Linear(
            in_features=1536, out_features=self.config["hparams"]["num_classes"]
        )
        ...
        self.train_acc = torchmetrics.classification.Accuracy(
            task="multiclass", num_classes=self.config["hparams"]["mlp"]["num_classes"]
        )
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.log(
            "training_loss", train_loss, prog_bar=True, on_step=True, on_epoch=True
        )
    ...
    def test_step(self, batch, batch_idx):
        images, labels = batch
        predictions = self.model(images)
        filepath_labels = self.config["exp"]["file_path_labels"][
    ...
    def on_test_epoch_end(self):
        ...
        per_classes_acc = {
            name: accuracy_score(
                np.array(self.test_labels) == i,
                np.array(self.test_prediction_labels) == i,
            )
        ...
        df.to_csv(
            f"{self.config['exp']['csv_dir']}/Test_seed_{self.seed}.csv", index=False
        )
    ...
    def on_predict_epoch_end(self):
        ...
        df = pd.DataFrame(self.csv_information, columns=columns)
        df.to_csv(
            f"{self.config['exp']['csv_dir']}/Predict_seed_{self.seed}.csv", index=False
        )
...
class EfficientNet_B0(pl.LightningModule):
    def __init__(self, config):
        ...
        self.model.classifier[1] = nn.Linear(
            in_features=1280, out_features=self.config["hparams"]["num_classes"]
        )
        ...
        self.train_acc = torchmetrics.classification.Accuracy(
            task="multiclass", num_classes=self.config["hparams"]["num_classes"]
        )
        self.valid_acc = torchmetrics.classification.Accuracy(
            task="multiclass", num_classes=self.config["hparams"]["num_classes"]
        )
        ...
        self.batch_size = self.config["hparams"]["batch_size"]
        self.seed = self.config["hparams"]["seed"]
        self.target_names = self.config["hparams"]["target_names"]
        self.save_hyperparameters()
        self.csv_information = []
...
class Swin_B(pl.LightningModule):
    def __init__(self, config):
        ...
        self.model.head = nn.Linear(
            self.num_features, self.config["hparams"]["num_classes"]
        )
...
class Restnet_50(pl.LightningModule):
    def __init__(self, config):
